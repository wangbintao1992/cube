...
!function() {
	...
	var c = function(b, c) {
		this.$element = a(b), this.options = c, a(b).data("nowtime", 0), a(b)
				.data("danmu_array", c.danmuss), a(b)
		...
		var d = this.$element.height(), e = parseInt(d / c.font_size_big), f = new Array;
		a("<div class='timer71452'></div>").appendTo(this.$element), this.$timer = a(".timer71452"), this.$timer
				.timer({
					delay : 100,
					repeat : c.sumtime,
					autostart : !1,
					callback : function() {
						if (d = a(b).height(), a(b).data("danmu_array")[a(b)
								.data("nowtime")])
							for (var g = a(b).data("danmu_array")[a(b)
									.data("nowtime")], h = 0; h < g.length; h++) {
								var i = "<div class='flying flying2' id='linshi'></div>";
								if (a(b).append(i), a("#linshi")
										.text(g[h].text), a("#linshi").css({
											color : g[h].color,
											...
											"font-weight" : "bold",
											"font-family" : "SimHei",
											"font-size" : c.font_size_big
										}), g[h].color < "#777777"
										&& a("#linshi").css({
											"text-shadow" : " 0px 0px 2px #FFFFFF"
										}), g[h].hasOwnProperty("isnew")
										&& a("#linshi").css({
													border : "2px solid "
															+ g[h].color
												}), 0 == g[h].size
										&& a("#linshi").css("font-size",
												c.font_size_small), 0 == g[h].position) {
									for (var j = parseInt(e * Math.random()); f
											.indexOf(j) >= 0;)
										var j = parseInt(e * Math.random());
									f.push(j), f.length == e
											&& (f = new Array, e = parseInt(d
													/ c.font_size_big));
									var k = j * c.font_size_big;
									a("#linshi").css({
												position : "absolute",
												top : k,
												left : c.width
								...
								} else if (1 == g[h].position) {
									var m = "top"
											+ parseInt(1e4 * Math.random())
													.toString();
									a("#linshi").attr("id", m), a("#" + m).css(
											{
												width : c.width,
												"text-align" : "center",
												position : "absolute",
												top : 5 + a(b).data("topspace")
											}), a(b).data(
											"topspace",
											a(b).data("topspace")
													+ c.font_size_big), a("#"
											+ m).fadeTo(
											c.top_botton_danmu_time,
											a(b).data("opacity"), function() {
												a(this).remove(), a(b)
														.data(
																"topspace",
																a(b)
								...
								} else if (2 == g[h].position) {
									var n = "top"
											+ parseInt(1e4 * Math.random())
													.toString();
									a("#linshi").attr("id", n), a("#" + n).css(
											{
												width : c.width,
												"text-align" : "center",
												position : "absolute",
	...
	}, c.prototype.danmu_start = function() {
		this.$timer.timer("start"), this.$element.data("paused", 0)
	}, c.prototype.danmu_stop = function() {
		this.$timer.timer("stop"), a(".flying").remove(), nowtime = 0, this.$element
				.data("paused", 1), this.$element.data("nowtime", 0)
	}, c.prototype.danmu_pause = function() {
		this.$timer.timer("pause"), a(".flying").pause(), this.$element.data(
				"paused", 1)
	}, c.prototype.danmu_resume = function() {
		this.$timer.timer("resume"), a(".flying").resume(), this.$element.data(
				"paused", 0)
	}, c.prototype.danmu_hideall = function() {
		a(".flying").css({
					opacity : 0
				})
	}, c.prototype.add_danmu = function(a) {
		this.$element.data("danmu_array")[a.time] ? this.$element
				.data("danmu_array")[a.time].push(a) : (this.$element
				.data("danmu_array")[a.time] = new Array, this.$element
				.data("danmu_array")[a.time].push(a))
